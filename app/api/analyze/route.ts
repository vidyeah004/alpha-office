...
export async function POST(request: Request) {
  try {
    ...
    if (!openRouterApiKey) {
      return NextResponse.json({ error: 'OpenRouter API Key is missing on Vercel deployment settings.' }, { status: 500 });
    }
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'google/gemini-2.5-flash', // Bulletproof, fast, and highly cost-effective model choice
        messages: [
          {
            role: 'system',
            ...
            CRITICAL: Return your response wrapped entirely in clean semantic HTML elements (such as <h3>, <p>, <ul>, <li>, <strong>, <table className="w-full border-collapse">, etc.). Do NOT wrap your response in markdown code fences (\`\`\`html) or raw markdown text. Output the inside HTML directly so it can be safely injected into a container.`
          },
          {
            role: 'user',
            content: userPrompt
    ...
    const htmlOutput = data.choices?.[0]?.message?.content || '<p>Error generating strategy output.</p>';
